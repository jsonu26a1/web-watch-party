addToLibrary({
    ...
    read_current_file: (ptr, offset, len) => Asyncify.handleAsync(async () => {
        ...
        // console.log(`call to read_current_file; ptr ${ptr}, size ${current_file.size}, rem ${remaining}, offset ${offset}, len ${len}`);
        if(remaining < len)
            len = remaining;
        ...
        let buf = await current_file.slice(offset, offset + len).arrayBuffer();
        // copy the data into the wasm runtime
        HEAPU8.set(new Uint8Array(buf), ptr);
        // console.log(`****** read_current_file; ptr ${ptr}, offset ${offset}, len ${len}, count ${buf.byteLength}`);
